...
export const uploadNewMetadata = async (name, description, image, purchasePrice, color, strength, rarity, powerLevel) => {
    const imageHash = await uploadFileToIPFS(image);
    const metadata = constructMetadata(name, description, imageHash, purchasePrice, color, strength, rarity, powerLevel);
